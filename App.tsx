    ...
    <div style={{padding:"120px 62px 90px",fontSize:9.5,lineHeight:1.5,position:"relative",minHeight:841,fontFamily:SANS,color:C.black,background:C.bg}}>
      ...
      {(type!=="contract"||(d.quoteRef||"none")==="table")&&<>
        ...
        {type==="amendment"
          ...
          :<div data-sig-anchor="true" style={{marginTop:22+(extraSigMargin||0)}}>
            <div style={{display:"flex",justifyContent:"flex-end",marginBottom:10}}>
              {d.retainer&&d.retMo&&type==="invoice"?(
                <div style={{width:175}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:3,fontSize:8}}><span style={{color:C.muted}}>{l?"Zwischensumme":"Subtotal"}</span><span>€ {Number(Math.round(linesSum/0.8)).toLocaleString("de-DE")}</span></div>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:3,fontSize:8,borderTop:`1px solid ${C.rule}`,paddingTop:4}}><span style={{color:C.muted}}>{l?"Retainer-Rabatt −20%":"Retainer discount −20%"}</span><span>−€ {Number(Math.round(linesSum/0.8)-linesSum).toLocaleString("de-DE")}</span></div>
                  <div style={{borderTop:`1px solid ${C.rule}`,paddingTop:6,display:"flex",justifyContent:"space-between",alignItems:"baseline"}}>
            ...
            <div key={pr.id} style={{border:`1px solid ${C.rule}`,borderRadius:2,padding:FS.pad,marginBottom:FS.gap,opacity:pr.paid?0.55:1}}>
              ...
              {(pr.qd||(pr.amendments||[]).length>0||(pr.renewals||[]).length>0)&&(
                <div style={{display:"flex",gap:isMobile?8:5,flexWrap:"wrap",borderTop:`1px solid ${C.rule}`,paddingTop:isMobile?10:7,marginBottom:isMobile?8:6}}>
                  ...
                  {["invoiced","paid"].includes(pr.status)&&pr.qd&&!pr.qd?.retainer&&<B v="sec" s={{fontSize:FS.docBtn,padding:isMobile?"9px 14px":"5px 10px"}} onClick={()=>openPDF(pr,"invoice","en",cl.id)}>Invoice</B>}
...
function ProjectsTab({clients,setClients,isMobile,onRevise,onGoToCalc,settings,rc,pendingProjectQNo,onPendingClear}: any) {
  ...
  const saveMonthlyInvoice=(cid: string,pid: string,inv: any)=>setClients((p: any[])=>p.map(c=>c.id!==cid?c:{...c,projects:c.projects.map((pr: any)=>pr.id!==pid?pr:{...pr,monthlyInvoices:[...(pr.monthlyInvoices||[]),inv]})}));
  ...
  if(renewT)return<RenewalModal p={renewT.p} onSave={(r: any)=>saveRenewal(renewT.cid,renewT.pid,r)} onClose={()=>{setRenewT(null);setScrollTrigger(t=>t+1);}} rc={rc} settings={settings}/>;
  if(pdf)return<PDFModal data={pdf.data} type={pdf.type} onClose={()=>{setPdf(null);setScrollTrigger(t=>t+1);}} settings={settings} onSave={(pdf.cid&&pdf.pid&&pdf.isRevision)?(doc: any)=>upP2(pdf.cid,pdf.pid,{qd:{...doc,contractRev:pdf.nextContractRev,clauses:doc.clauses||[]}}):(pdf.cid&&pdf.pid&&pdf.isMonthlyInv)?(doc: any)=>{if(pdf.monthlyIdx!==undefined){setClients((p: any[])=>p.map(c=>c.id!==pdf.cid?c:{...c,projects:c.projects.map((pr: any)=>pr.id!==pdf.pid?pr:{...pr,monthlyInvoices:(pr.monthlyInvoices||[]).map((inv: any,ii: number)=>ii===pdf.monthlyIdx?{...inv,doc,iNo:doc.iNo||inv.iNo,delivery:doc.delivery,amount:doc.total||(doc.lines||[]).reduce((s: number,l: any)=>s+(parseFloat(l.amt)||0),0)}:inv)})}));}else{const inv={id:uid(),iNo:doc.iNo||doc.rNo,delivery:doc.delivery,amount:doc.total||(doc.lines||[]).reduce((s: number,l: any)=>s+(parseFloat(l.amt)||0),0),paid:false,doc};saveMonthlyInvoice(pdf.cid,pdf.pid,inv);}}:(pdf.cid&&pdf.pid)?(doc: any)=>{const tot=doc.total||(doc.lines||[]).reduce((s: number,l: any)=>s+(parseFloat(l.amt)||0),0);upP2(pdf.cid,pdf.pid,{qd:{...doc,clauses:doc.clauses||[]},amount:tot});}:undefined}/>;
        ...
        {isOpen&&<div style={{padding:FS.pad,paddingTop:4,marginBottom:FS.gap,background:"transparent"}}>
          ...
          <div style={{display:"flex",gap:isMobile?8:5,flexWrap:"wrap",marginBottom:isMobile?10:7}}>
            ...
            {pr.qd?.retainer&&(pr.monthlyInvoices||[]).map((inv: any,ii: number)=>(
              inv.doc&&<B key={ii} v="sec" s={{fontSize:FS.docBtn,padding:isMobile?"9px 14px":"5px 10px",color:inv.paid?C.black:C.green,borderColor:inv.paid?C.rule:C.greenBorder}} onClick={()=>setPdf({cid:cl.id,pid:pr.id,data:inv.doc,type:"invoice",lang:"en",isMonthlyInv:true,monthlyIdx:ii})}>Invoice M{String(ii+1).padStart(2,"0")}</B>
            ))}
          </div>
          <div style={{display:"flex",gap:isMobile?8:5,flexWrap:"wrap",alignItems:"center",paddingTop:isMobile?10:7,borderTop:`1px solid ${C.rule}`}}>
            ...
            {pr.status==="production"&&(pr.qd?.retainer?(()=>{const mis=pr.monthlyInvoices||[];const retMo=pr.qd?.retMo||1;const nextN=mis.length+1;const allDone=mis.length>=retMo;const lastPaid=mis.length>0&&mis[mis.length-1].paid;const canNext=mis.length===0||lastPaid;return(<>{mis.map((inv: any,ii: number)=>(<span key={inv.id||ii} style={{display:"contents"}}>{!inv.paid&&<B v="sec" s={{fontSize:FS.actionBtn,color:C.green,borderColor:C.greenBorder,padding:isMobile?"10px 14px":"7px 14px"}} onClick={()=>{const newMis=mis.map((inv2: any,i2: number)=>i2===ii?{...inv2,paid:true}:inv2);const allPaid=newMis.length>=retMo&&newMis.every((v: any)=>v.paid);setClients((p: any[])=>p.map(c=>c.id!==cl.id?c:{...c,projects:c.projects.map((proj: any)=>proj.id!==pr.id?proj:{...proj,monthlyInvoices:newMis,paid:allPaid,status:allPaid?"paid":proj.status})}));}}>Mark M{ii+1} Paid</B>}{<B v="sec" s={{fontSize:FS.actionBtn,color:C.amber,padding:isMobile?"10px 14px":"7px 14px"}} onClick={()=>setClients((p: any[])=>p.map(c=>c.id!==cl.id?c:{...c,projects:c.projects.map((proj: any)=>proj.id!==pr.id?proj:{...proj,monthlyInvoices:(proj.monthlyInvoices||[]).filter((_: any,i2: number)=>i2!==ii),paid:false,status:"production"})}))}>Undo M{ii+1}</B>}</span>))}{!allDone&&canNext&&<B s={{fontSize:FS.actionBtn,padding:isMobile?"10px 18px":"7px 14px"}} onClick={()=>{const q=pr.qd;const baseINo=`INV-${(q?.qNo||"").replace("QUO","").trim()||"001"}`;const iNo=`${baseINo}-M${String(nextN).padStart(2,"0")}`;const skip=["usage","excl","rush","revision","whitelisting","aspect","raw footage","kill","pinned","link in bio"];const monthlyLines=(q?.lines||[]).map((l: any)=>({...l,amt:skip.some((s: string)=>(l.name||"").toLowerCase().includes(s))?parseFloat(l.amt)||0:Math.round((parseFloat(l.amt)||0)*0.8),up:parseFloat(l.up)||0}));const monthlyAmt=Math.round(monthlyLines.reduce((s: number,l: any)=>s+(parseFloat(l.amt)||0),0));const data={brand:q?.brand,contact:q?.contact,date:today(),qNo:q?.qNo,iNo,delivery:today(),ctype:q?.ctype||"Content Creator",lines:monthlyLines,total:monthlyAmt,retainer:true,retMo:q?.retMo,footer:"Thank you for the pleasure of working together."};setPdf({cid:cl.id,pid:pr.id,data,type:"invoice",lang:"en",isMonthlyInv:true});}}>Create Invoice {nextN}/{retMo}</B>}</>);})():(<B s={{fontSize:FS.actionBtn,padding:isMobile?"10px 18px":"7px 14px",opacity:pr.deliveryDate?1:0.35,cursor:pr.deliveryDate?"pointer":"not-allowed"}} title={pr.deliveryDate?"":"Set a delivery date first"} onClick={()=>{if(!pr.deliveryDate)return;setStatus(cl.id,pr.id,"invoiced");openPDF({...pr,status:"invoiced"},"invoice","en",cl.id);}}>Create Invoice</B>))}
                ...
                <div key={item.id} style={{borderBottom:`1px solid ${C.rule}`,opacity:isDone?0.6:1}}>
                    ...
                    <div style={{padding:"11px 0"}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
                        <div style={{display:"flex",alignItems:"center",gap:7,minWidth:0,flex:1}}>
                          <span style={{fontSize:10,fontWeight:"500",color:C.black,letterSpacing:"0.07em",textTransform:"uppercase" as const,flexShrink:0}}>{item.clientName}</span>
                          <span style={{fontSize:10,padding:"1px 7px",border:`1px solid ${catStyle.border}`,borderRadius:10,color:catStyle.color,background:catStyle.bg,flexShrink:0,letterSpacing:"0.04em"}}>{item.category}</span>
                        </div>
                        <span style={{fontSize:11,color:dlColor,fontWeight:dlColor===C.red?"600":"400",flexShrink:0,marginLeft:8}}>{fmtDeadline(item.deadline)}</span>
                      </div>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                        <div style={{flex:1,minWidth:0,marginRight:12}}>
                          {isEditing?(
                            <input autoFocus value={editingVal} onChange={e=>setEditingVal(e.target.value)}
                              ...
                              style={{fontFamily:SANS,fontSize:13,color:C.black,border:"none",borderBottom:`1px solid ${C.black}`,background:"transparent",outline:"none",padding:"0 0 1px",width:"100%"}}/>
                          ):(
                            <span onClick={()=>{if(!isDone){setEditingId(item.id);setEditingVal(item.name);}}}
                              style={{fontSize:13,color:C.black,cursor:isDone?"default":"text",textDecoration:isDone?"line-through":"none",display:"block",marginBottom:2}}>{item.name}</span>
                          )}
                          {item.lineNote&&<span style={{fontSize:11,color:C.muted,display:"block",marginBottom:2}}>{item.lineNote}</span>}
                          {item.notes&&!isNoting&&<span onClick={()=>{setNoteId(item.id);setNoteVal(item.notes);}} style={{fontSize:11,color:C.amber,display:"block",marginBottom:2,cursor:"pointer"}}>{item.notes}</span>}
                          {!item.notes&&!isNoting&&!isDone&&<button onClick={()=>{setNoteId(item.id);setNoteVal("");}} style={{fontSize:11,color:C.light,background:"none",border:"none",cursor:"pointer",padding:0,fontFamily:SANS,letterSpacing:"0.02em"}}>+ note</button>}
                          {isCreated&&mgrStatus&&<span style={{fontSize:11,color:mgrStatus.color,display:"block",marginTop:1,letterSpacing:"0.02em"}}>{mgrStatus.label} · {fmtD(mgrStatus.date)}</span>}
                          {dayTag&&<span style={{fontSize:11,color:C.muted,display:"inline-block",marginTop:3,padding:"1px 5px",border:`1px solid ${C.rule}`,borderRadius:2,background:C.white}}>{dayTag}</span>}
                        </div>
                        <div style={{display:"flex",gap:12,flexShrink:0}}>
                          <div style={{display:"flex",flexDirection:"column" as const,alignItems:"center",gap:3}}>
                            <span style={{fontSize:11,color:C.light,letterSpacing:"0.04em"}}>created</span>
                          ...
                          {showPost&&(
                            <div style={{display:"flex",flexDirection:"column" as const,alignItems:"center",gap:3}}>
                              <span style={{fontSize:11,color:C.light,letterSpacing:"0.04em"}}>posted</span>
                  ...
                  <div style={{display:"flex",alignItems:"flex-start",gap:10,padding:"9px 0"}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      {!item.notes&&!isNoting&&!isDone&&<button onClick={()=>{setNoteId(item.id);setNoteVal("");}} style={{fontSize:10,color:C.light,background:"none",border:"none",cursor:"pointer",padding:0,fontFamily:SANS,marginTop:2,letterSpacing:"0.02em"}}>+ note</button>}
                    ...
                    <div style={{display:"flex",flexDirection:"column" as const,alignItems:"flex-end",gap:4,flexShrink:0}}>
                      <span style={{fontSize:11,color:dlColor,fontWeight:dlColor===C.red?"600":"400"}}>{fmtDeadline(item.deadline)}</span>
...
function CreatorProjects({clients,isMobile}: {clients:any[],isMobile:boolean}) {
  ...
  clients.forEach((c: any)=>{
    (c.projects||[]).forEach((pr: any)=>{
      allProjects.push({...pr,clientName:c.name,clientId:c.id});
    });
  });
  ...
  return(
    <div>
      ...
      {active.length>0&&<>
        <p style={{fontSize:9,color:C.muted,letterSpacing:"0.08em",textTransform:"uppercase" as const,margin:"0 0 8px"}}>Active — {active.length}</p>
        {active.map(pr=>renderCard(pr,false))}
      </>}

      {done.length>0&&<div style={{marginTop:active.length>0?24:0}}>
        <p style={{fontSize:9,color:C.muted,letterSpacing:"0.08em",textTransform:"uppercase" as const,margin:"0 0 8px"}}>Done — {done.length}</p>
        {done.map(pr=>renderCard(pr,true))}
      </div>}
    </div>
  );
