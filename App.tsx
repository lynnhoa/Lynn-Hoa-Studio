...
const mw=mobile?window.open(””,”*blank”):null;
...
const dateStr=(preview.date||new Date().toISOString().slice(0,10)).replace(/-/g,”*”);
...
{!pr.qd&&<B s={{fontSize:FS.docBtn,padding:isMobile?“9px 14px”:“5px 10px”}} onClick={()=>onGoToCalc(pr.*cname)}>+ Create Quote</B>}
{pr.qd&&<B v=“sec” s={{fontSize:FS.docBtn,padding:isMobile?“9px 14px”:“5px 10px”}} onClick={()=>openPDF(pr,“quote”,“en”,cl.id)}>{pr.qd.rev>0?`Quote R${pr.qd.rev}`:“Quote”}</B>}
{[“contracted”,“production”,“invoiced”,“paid”].includes(pr.status)&&pr.qd&&<B v=“sec” s={{fontSize:FS.docBtn,padding:isMobile?“9px 14px”:“5px 10px”}} onClick={()=>openPDF(pr,“contract”,“en”,cl.id)}>{pr.qd.contractRev>0?`Contract R${pr.qd.contractRev}`:“Contract”}</B>}
...
{!r.paid&&<B v=“sec” s={{fontSize:FS.actionBtn,color:C.amber,padding:isMobile?“10px 14px”:“7px 14px”}} onClick={()=>setClients((p: any[])=>p.map(c=>c.id!==cl.id?c:{…c,projects:c.projects.map((proj: any)=>proj.id!==pr.id?proj:{…proj,renewals:proj.renewals.filter((*: any,i: number)=>i!==ri)})}))}>Undo Renewal {ri+1}</B>}
