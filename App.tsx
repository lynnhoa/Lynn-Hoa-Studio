        ...
        {isOpen&&<div style={{padding:FS.pad,paddingTop:4,marginBottom:FS.gap,background:"transparent"}}>
          ...
          <div style={{display:"flex",gap:isMobile?8:5,flexWrap:"wrap",marginBottom:isMobile?10:7}}>
            ...
            {pr.qd?.retainer&&(pr.monthlyInvoices||[]).map((inv: any,ii: number)=>(
              inv.doc&&<B key={ii} v="sec" s={{fontSize:FS.docBtn,padding:isMobile?"9px 14px":"5px 10px",color:inv.paid?C.black:C.green,borderColor:inv.paid?C.rule:C.greenBorder}} onClick={()=>setPdf({cid:cl.id,pid:pr.id,data:inv.doc,type:"invoice",lang:"en"})}>Invoice M{String(ii+1).padStart(2,"0")}</B>
            ))}
          </div>
