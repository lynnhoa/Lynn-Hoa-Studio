...
function EditRow({draft,setDraft,onSave,onCancel}: any) {
  const priceStr = (draft.p === null || draft.p === undefined) ? "" : String(draft.p);
  function onName(e: any) { setDraft((p: any)=>({...p,n:e.target.value})); }
  function onNote(e: any) { setDraft((p: any)=>({...p,note:e.target.value})); }
  function onPrice(e: any) {
    const v = e.target.value;
    setDraft((p: any)=>({...p,p:v===""?null:parseFloat(v)||0}));
  }
  return(
    <div style={{padding:"10px 12px"}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 80px",gap:8,marginBottom:8}}>
        <div><Lbl>Name</Lbl><I value={draft.n} onChange={onName}/></div>
        <div><Lbl>Note</Lbl><I value={draft.note||""} onChange={onNote}/></div>
        <div><Lbl>Price</Lbl><I type="number" value={priceStr} onChange={onPrice}/></div>
      </div>
      <div style={{display:"flex",gap:6}}>
        <B onClick={onSave} s={{fontSize:8}}>Save</B>
        <B v="sec" onClick={onCancel} s={{fontSize:8}}>Cancel</B>
      </div>
    </div>
  );
}
...
export function ItemPool({pool,setPool}: {pool: any[], setPool: (fn: any)=>void}) {
  ...
  function startEdit(it: any) { setEditId(it.id); setDraft({...it}); }
  ...
  return(
    <div>
      ...
      <div style={{border:`1px solid ${C.rule}`,borderRadius:2,marginBottom:12}}>
        ...
        {items.length===0&&<p style={{fontSize:11,color:C.light,padding:"16px 12px",margin:0}}>No items yet.</p>}
        {items.map((it: any)=>(
          <div key={it.id} style={{borderBottom:`1px solid ${C.rule}`}}>
            {editId===it.id&&draft
              ? <EditRow draft={draft} setDraft={setDraft} onSave={saveEdit} onCancel={cancelEdit}/>
      ...
      <div style={{marginTop:20,padding:"11px 14px",background:C.amberBg,border:`1px solid ${C.amberBorder}`,borderRadius:2}}>
        <p style={{fontSize:10.5,color:C.amber,margin:0}}>Changes here update the calculator instantly.</p>
      </div>
    </div>
  );
}
